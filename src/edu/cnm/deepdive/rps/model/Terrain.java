...
public void step(){
  ...
  /**
   * Returns a reference to the terrain contents. <strong>Impotant!</strong>
  ...
  private Location normalize(Location base, Location offset){
    int row = (base.getRow() + offset.getRow()) % grid.length;
    //int row = (base.getRow() + offset.getRow() + grid.length) % grid.length;
    int col = (base.getColumn() + offset.getColumn()) % grid.length;
